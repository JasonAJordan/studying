...
viewValidation model =
  if String.length (model.password) <= 6 then
    ...
    div  [ style "color" "red" ] [ text "Password needs a number" ]
  else if not (String.any isUpper model.password) then
    div  [ style "color" "red" ] [ text "Password needs a uppercase" ]
  else if not (String.any isLower model.password) then
    div  [ style "color" "red" ] [ text "Password needs a lowercase" ]
